...
export function pickDailyQuestions(weekIndex, runSeed = 1, count = DAYS_PER_WEEK, exclude = null) {
  const seed = (runSeed ^ ((weekIndex + 1) * 0x9e3779b1)) >>> 0;
  const rand = mulberry32(seed);
  const band = difficultyBand(weekIndex);
  const notOpening = (q) => weekIndex <= 2 || !OPENING_RE.test(q.scenario); // 開學題鎖前三週
  const yr = (q) => YEARROUND_CATS.includes(q.cat);
  const fresh = (q) => !exclude || !exclude.has(q.id);
  // 三層遞補：難度合身且未出過 → 放寬難度且未出過 → 最後才允許重複，避免抽不滿 5 題
  let pool = LEGAL_POOL.filter((q) => yr(q) && band.includes(q.diff) && notOpening(q) && fresh(q));
  if (pool.length < count) pool = LEGAL_POOL.filter((q) => yr(q) && notOpening(q) && fresh(q));
  if (pool.length < count) pool = LEGAL_POOL.filter((q) => yr(q) && notOpening(q));
...
function pickByCategory(weekIndex, runSeed, cats, count = DAYS_PER_WEEK, opts = {}) {
  const { subs = null, matchRe = null, exclude = null } = opts;
  ...
  let arr = shuffle(primary.filter(fresh));
  const fill = (src) => {
    const seen = new Set(arr.map((q) => q.id));
    arr = arr.concat(shuffle(src.filter((q) => !seen.has(q.id) && fresh(q))));
  };
  if (arr.length < count) fill(inCat);          // 補同分類（去子類限制、仍避開已出過）
  if (arr.length < count) fill(LEGAL_POOL);      // 仍不足才補全池（仍避開已出過）
  if (arr.length < count) {                      // 最後關頭才允許重複，避免抽不滿
    const seen = new Set(arr.map((q) => q.id));
    arr = arr.concat(shuffle(inCat.filter((q) => !seen.has(q.id))));
  }
  return arr.slice(0, count);
}
...
function drawWeekItems(weekIndex, runSeed, exclude) {
  if (EXAM_THEME_WEEKS.includes(weekIndex)) {
    return pickByCategory(weekIndex, runSeed, EXAM_CATS, DAYS_PER_WEEK, { exclude });
  }
  if (EVENT_WEEKS.includes(weekIndex)) {
    return pickByCategory(weekIndex, runSeed, EVENT_CATS, DAYS_PER_WEEK, { exclude });
  }
  const act = ACTIVITY_WEEKS[weekIndex];
  if (act) {
    return pickByCategory(weekIndex, runSeed, act.cats, DAYS_PER_WEEK,
      { subs: act.subs, matchRe: act.match, exclude });
  }
  return pickDailyQuestions(weekIndex, runSeed, DAYS_PER_WEEK, exclude);
}

// 整局一次排定：依序走過 40 週，用同一個 used 集合確保「整局題目不重複」。
// 事件卡同樣跨週去重。以 runSeed 為快取鍵：同局穩定、存檔重載一致、新局換種子＝全新一輪。
const _scheduleCache = new Map();
function buildYearSchedule(runSeed) {
  ...
  for (let w = 0; w < TOTAL_WEEKS; w++) {
    ...
    const items = drawWeekItems(w, runSeed, usedQ);
    items.forEach((q) => usedQ.add(q.id));
    let card = null;
    if (EVENT_CARD_WEEKS[w] != null) {
